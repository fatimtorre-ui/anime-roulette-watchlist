...
export function useAnimeRoulette() {
   ...
   const spin = async () => {
    ...
    try{
        for (let attempt = 0; attempt < MAX_SAFE_SPIN_ATTEMPTS; attempts++) {
            ...
            if (!response.ok) {
                error.value = `Something went wrong while contacting Jinkan. Status: ${response.status}.`
                return
            }
            ...
            if (!candidateAnime) {
                error.value = 'Jinkan returned an empty anime payload. Try spinning again.'
                return
            }
